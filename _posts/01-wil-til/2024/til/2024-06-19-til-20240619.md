        ...
        return this.webClient1.post()
            ...
            .onErrorResume(throwable -> {
                ...
                return webClient2.post()
                  ...
                  .doOnSuccess(map -> {handleDoOnSuccess(value, map);}) // 2server: 성공시 처리되는 로직
                  .doOnError(ApiClient::handleDoOnError); // 2server: 통신은 성공했으나 처리중의 오류 처리 로직
            })
           .doOnSuccess(map -> {handleDoOnSuccess(value, map);}) // 1server: 성공시 처리되는 로직
           .doOnError(ApiClient::handleDoOnError) // 1server: 통신은 성공했으나 처리중의 오류 처리 로직
           .then();
```
